...
for index, row in gold_df.iterrows():
    ...
    class_code = get_geo_class(geoname_id, filename)
    ...
    file_id_to_toponyms[file_id].append([toponym,geoname_id, class_code])
                ...
                if file_id in file_id_to_toponyms:
                    ...
                    for entry in file_id_to_toponyms[file_id]:
                        ...
                        if not remaining_text:
                            ...
                            search_for = remaining_text[first_index_toponym:]
                            first_index_real = file_contents.find(search_for)
                            last_index_real = first_index_real + len(toponym)
                            last_index_toponym = first_index_toponym + len(toponym)
                            ...
                            new_row = {
                                ...
                                "toponym": toponym,
                                "geoname_id": geoname_id,
                                "first_index_toponym": first_index_real,
                                "last_index_toponym": last_index_real,
                                "class_code": class_code
                            }
                            new_df = new_df._append(new_row, ignore_index=True)
...
new_df.to_csv("annotations_gold_index.tsv", sep="\t", index=False)